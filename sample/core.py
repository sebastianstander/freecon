...
class fcSuite :
	def __init__(self) 
		self.tskq = []										# task_queue
		self.current_tsk = 0							# current_index
		
		# Buffers
		self.buffers = { "default","" }			# data_buffer
		self.bf_sel = 0									# selected_buffer
		self.bf_count = 0								# buffer_count

		# Flags
		self.prompt_user = true 
		self.backup_buffers = false
		self.stdout_feeback = true
		self.task_directory = {	0 , exit_suite 
												1 , toggle_feedback
	...
	def __exit__ (self)  :
		...
		if ( self.buffer_backup is True ) :
			self.backup_buffers()
	def interpert_taskflow(self) :
		# Task Source
		# If on CLI "file:"
